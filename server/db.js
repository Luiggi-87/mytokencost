...
async function initializeTables() {
  ...
  await dbRun(`CREATE TABLE IF NOT EXISTS apis (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, name TEXT NOT NULL, type TEXT NOT NULL, api_key TEXT, base_url TEXT, pricing_model TEXT, unit_cost NUMERIC(10,2), model TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users(id))`);
  await runIgnoringErrors(`ALTER TABLE apis ADD COLUMN model TEXT`);
  await runIgnoringErrors(`ALTER TABLE apis ADD COLUMN project_id TEXT`);
  await dbRun(`CREATE TABLE IF NOT EXISTS companies (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, name TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users(id))`);
  await dbRun(`CREATE TABLE IF NOT EXISTS clients (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, company_id TEXT NOT NULL, name TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users(id), FOREIGN KEY (company_id) REFERENCES companies(id))`);
  ...
  await runIgnoringErrors(`ALTER TABLE projects ADD COLUMN company_id TEXT`);
  await runIgnoringErrors(`ALTER TABLE projects ADD COLUMN client_id TEXT`);
